...
module HigherComputability.NeutralRecursive.ParallelSearch
  {ℓbase : Level}
  (M : {ℓ : Level} → Type ℓ → Type (ℓ-max ℓbase ℓ))
  ⦃ _ : ModalOperator ℓbase M ⦄
  where

open import HigherComputability.NeutralRecursive.ModalMarkovsPrinciple {ℓbase = ℓbase} M

parallelSearch : ⦃ _ : MarkovInduction ℓ ⦄
  (P Q : ℕ → Type ℓ) (decP : (n : ℕ) → M (Dec (P n)))
  (decQ : (n : ℕ) → M (Dec (Q n))) →
  NonEmpty (Σ ℕ P ⊎ Σ ℕ Q) → M (Σ ℕ P ⊎ Σ ℕ Q)
parallelSearch {ℓ} P Q decP decQ ¬¬exists = do
  (n , p⊎q) ← search PQ decPQ ¬¬existsPQ
